...
bool ehprimo(int n) // Função que verifica se um número são primo
{
    if (n <= 1) // Números menores ou iguais a 1 não são primos
        return false;
    for (int i = 2; i * i <= n; i++) // Verifica se o n?mero são divisível por algum número entre 2 e a raiz quadrada de n
    {
        if (n % i == 0) // Se for divisível, não são primo
            return false;
    }
